...
export function MatriculaTable({ courses }) {
  return (
        ...
        <table className="w-full border-collapse rounded-lg overflow-hidden">
          <thead>
            <tr className="bg-indigo-500 text-white rounded-t-lg">
              ...
              <th className="p-3 text-center">Pos. Relativa</th>
          ...
          <tbody className="bg-white rounded-b-lg">
            {courses.map((course, index) => (
              ...
              >
                ...
                <td className={`p-3 font-bold text-center ${course.LineaInscripcion < course.total ? 'text-green-500' : 'text-red-500'}`}>
                  {course.LineaInscripcion}/{course.total}
                </td>
                <td className="p-3">{course.docente}</td>
                <td className="p-3 text-center">{course.sesiones}</td>
                <td className="px-12 text-center py-4 whitespace-nowrap text-sm text-black-500">
                    <button className="transform transition-transform duration-100 active:scale-95 hover:scale-105">
                        <img className='w-8 h-8 mx-auto' src={trash_can} alt="svg" />
                    </button>
                </td>
              </tr>
            ))}
